...
对于LoginSteps类，我们无需做任何更改。当我们运行RunCucumberTest类时，我们可以看到测试成功，并且测试执行了两遍，针对于两组数据。这使得定义大量示例、边缘情况和特殊结果变得非常容易。变量不是硬编码测试数据，而是在Examples部分定义，并在Scenario Outline部分中使用。
